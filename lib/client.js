window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* @returns the constructed Notification (tests assert on it).
		*/
		function fireSessionNotification(kind, t, target) {
			switch (kind) {
				case "approval": return show(titled(t("notify.approval.title"), target.label), t("notify.other.approval.body"), target.tag, target);
				case "question": return show(titled(t("notify.question.title"), target.label), t("notify.other.question.body"), target.tag, target);
				case "done": return show(titled(t("notify.sessionDone.title"), target.label), t("notify.other.done.body"), target.tag, target);
			}
		}
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			*/
			const listNotified = /* @__PURE__ */ new Map();
			...
			*/
			const scanList = () => {
				...
				for (const sid of list.ids) {
					...
					if (sid !== current && status !== void 0) {
						const kind = status === "approval" ? "approval" : "question";
						const guard = listNotified.get(sid);
						if (guard === void 0 || !guard.has(kind)) {
							if (guard === void 0) listNotified.set(sid, new Set([kind]));
							else guard.add(kind);
							if (hiddenNow() && notificationUsable()) fireSessionNotification(kind, t, {
								label: labelOf(sid),
								onOpen: openOf(sid),
								tag: `${sid}:${kind}`
							});
						}
					} else if (status === void 0) listNotified.delete(sid);
					if (sid !== current && summary.completed === true) {
						if (!completedNotified.has(sid)) {
							completedNotified.add(sid);
							if (hiddenNow() && notificationUsable()) fireSessionNotification("done", t, {
								label: labelOf(sid),
								onOpen: openOf(sid),
								tag: `${sid}:done`
							});
				...
				for (const sid of listNotified.keys()) if (list.byId[sid] === void 0) listNotified.delete(sid);
