	...
	factory: (require) => {
		...
		*/
		function fireNotification(wait, t, target) {
			return show(titled(wait.kind === "approval" ? t("notify.approval.title") : t("notify.question.title"), target.label), wait.kind === "approval" ? wait.reason ?? t("notify.approval.body", { toolName: wait.toolName }) : (() => {
			...
			*/
			...
			const scan = () => {
				try {
					...
					if (snapshot === void 0 || snapshot.legacy === void 0) return;
					...
					for (const turn of snapshot.legacy.turnEnds.keys()) {
						...
						turns.add(turn);
						if (hiddenNow() && notificationUsable()) fireTurnNotification(turn, turnSummaryOf(snapshot.legacy.nodes, turn), t, {
