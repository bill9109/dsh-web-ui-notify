...
function withClickFocus(notification: Notification): Notification {
 ...
 */
export function fireNotification(wait: PendingInteraction, t: Translate): Notification | undefined {
 ...
 * @returns the constructed Notification (tests assert on it).
 */
export function fireTurnNotification(turn: number, summary: string | undefined, t: Translate): Notification | undefined {
  const body = summary !== undefined && summary !== ''
    ? summary
    : t('notify.turn.body', { turn: String(turn) })
  return withClickFocus(new Notification(
    t('notify.turn.title'),
    { body, tag: `turn:${turn}`, requireInteraction: true },
  ))
}
